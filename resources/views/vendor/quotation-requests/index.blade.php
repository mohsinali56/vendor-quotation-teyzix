<x-app-layout>
    <style>
        ...
        .vq-btn-edit:hover { background: #eceae4; border-color: #d0cfc9; }
        .vq-btn-edit svg {
            width: 13px;
            height: 13px;
            stroke: currentColor;
        ...
        .vq-btn-submit-quote:hover { background: #0f3460; }
        .vq-btn-submit-quote svg {
            width: 13px;
            height: 13px;
            stroke: currentColor;
            fill: none;
        ...
        @media (max-width: 640px) {
            ...
            .vq-btn-submit-quote, .vq-btn-edit { width: 100%; justify-content: center; }
        }
    ...
    <div class="vq-page">
        <div class="vq-container">
            ...
            <div class="vq-page-header">
                <div>
                    <div class="vq-page-title">My Quotation Requests</div>
                    <div class="vq-page-sub">All quotation requests assigned to you</div>
                </div>
                <div class="vq-count-pill">
                    ...
                    {{ $quotations->total() }} Request{{ $quotations->total() != 1 ? 's' : '' }}
                </div>
            </div>

            <div class="vq-cards">
                @forelse($quotations as $req)
                    @php
                        $vendorPivot = $req->vendors->where('id', auth()->id())->first()->pivot;
                        $statusClass = match($vendorPivot->status) {
                            'submitted' => 'submitted',
                            'approved'  => 'approved',
                            'rejected'  => 'rejected',
                            default     => 'pending',
                    ...
                    <div class="vq-card">
                        ...
                        <div class="vq-card-bottom">
                            ...
                            <div class="vq-badges-row">
                                <span class="vq-status {{ $statusClass }}">
                                    <span class="vq-status-dot"></span>
                                    {{ ucfirst($vendorPivot->status) }}
                                </span>
                                @if($vendorPivot->amount)
                                    <span class="vq-amount-badge">
                                        <svg viewBox="0 0 24 24" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                                            <line x1="12" y1="1" x2="12" y2="23"/>
                                            <path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/>
                                        </svg>
                                        PKR {{ number_format($vendorPivot->amount) }}
                                    </span>
                                @endif
                            </div>

                            <div class="vq-actions-row">
                                <a href="{{ route('vendor.quotation-requests.edit', $req->id) }}" class="vq-btn-edit">
                                    <svg viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                        <path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/>
                                        <path d="M18.5 2.5a2.121 2.121 0 013 3L12.5 14.5l-4 1 1-4 9-9z"/>
                                    </svg>
                                    Edit
                                </a>

                                @if($vendorPivot->status == 'pending')
                                    <a href="{{ route('vendor.quotation-requests.edit', $req->id) }}" class="vq-btn-submit-quote">
                                        <svg viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
